...
export interface ToolDef {
  ...
  functional: boolean;
  comingSoon?: boolean;
  /** true => tool is temporarily down for technical reasons (distinct from
   * "coming soon"); generation is disabled and a maintenance screen shows. */
  maintenance?: boolean;
...
const BUILTIN_OPTION_PROMPTS: Record<string, string> = {
...
export function resolveOptionPrompt(
  toolPrompts: Record<string, string>,
  toolId: string,
  settingId: string,
  optionId: string
): string | undefined {
  const key = optionKey(toolId, settingId, optionId);
  return toolPrompts[key] ?? BUILTIN_OPTION_PROMPTS[key];
}
...
export function composeToolPrompt(
  ...
  for (const s of visibleSettings(tool, selections)) {
    const optId = selections[s.id] ?? s.default;
    const frag = resolveOptionPrompt(toolPrompts, tool.id, s.id, optId);
