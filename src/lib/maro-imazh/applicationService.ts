...
export interface MaroImageApplicationAdapter<T> {
  failure(payload: Record<string, unknown>, status: number): T;
  stream(run: (send: (payload: Record<string, unknown>) => void) => Promise<void>): T;
}
...
export async function executeMaroImageApplication<T>(
  req: Request,
  body: AiImageRequest,
  adapter: MaroImageApplicationAdapter<T>
): Promise<T> {
  if (!hasOpenAiKey()) {
    return adapter.failure({ error: "no-key" }, 503);
  }

  const infraDeny = denyIfProductionWithoutSupabase();
  if (infraDeny) return adapter.failure({ error: "service_unavailable" }, infraDeny.status);

  const tool = getTool(body?.toolId ?? "");
  if (!tool || tool.kind !== "image") {
    return adapter.failure({ error: "bad-tool" }, 400);
  }
  if (!body?.prompt?.trim()) {
    return adapter.failure({ error: "missing-prompt" }, 400);
  }
  ...
  if (
    ...
    return adapter.failure({ error: "invalid_image_reference" }, 400);
  }
  ...
  if (declaredAttachments.some((value) => value.startsWith("storage:generations/"))) {
    ...
    if (!referenceOwner) return adapter.failure({ error: "unauthorized" }, 401);
    for (const storageRef of declaredAttachments) {
      ...
      try {
        ...
        return adapter.failure({ error: code }, status);
      }
    }
  }
  ...
  if (supabaseServerConfigured()) {
    ...
    try {
      ...
      if (e instanceof GenerationGuardError) {
        return adapter.failure(
          { error: e.code, message: e.message, ...e.extra },
          e.status
        );
      }
      return adapter.failure({ error: "internal" }, 500);
    }
  }
  ...
  for (const attachment of body.attachments ?? []) {
    ...
    if (attachment.startsWith("data:image/")) {
      ...
      if (!validated.ok) {
        return adapter.failure({ error: validated.reason }, 400);
      }
    ...
    if (attachment.startsWith("storage:generations/")) {
      ...
      if (!resolved) return adapter.failure({ error: "reference_not_found" }, 404);
  ...
  // The adapter chooses transport behavior. The current UI adapter emits SSE
  // heartbeats while Railway waits for image generation to finish.
  return adapter.stream(async (send) => {
