...
import { publishSystemPromptVersion } from "@/lib/engine/promptVersions";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const live = await publishSystemPromptVersion(id, auth.admin.userId);
    await writeAuditEvent({
      actorId: auth.admin.userId,
      action: "engine.system_prompt.publish",
