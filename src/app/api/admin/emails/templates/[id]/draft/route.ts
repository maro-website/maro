...
import { ensureEditableDraft } from "@/lib/email/cms";
...
export async function POST(
  ...
  try {
    ...
    const draft = await ensureEditableDraft(id, auth.admin.userId);

    await writeAuditEvent({
      actorId: auth.admin.userId,
      action: "email.template.draft_created",
      targetType: "email_template_versions",
      targetId: draft.id,
      requestId: auth.requestId,
      metadata: { template_id: id, version_label: draft.versionLabel },
    });
